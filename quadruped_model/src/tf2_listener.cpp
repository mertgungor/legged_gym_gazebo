...
class PointcloudTransformer : public rclcpp::Node
{
    ...
    void pointcloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) const
    {
        ...
        pcl::fromROSMsg(transformed_pointcloud, *cloud);
        ...
        for(pcl::PointCloud<pcl::PointXYZ>::iterator it = cloud->begin(); it != cloud->end(); ++it)
        {
            ...
            x = (it->x / 10 + 5);
            y = (it->y / 10 + 8);
            std::cout << "X: " << x << " Y: " << y << " Z: " << it->z << std::endl;
            // RCLCPP_INFO(this->get_logger(), "Z: %f", it->z);
            // printf("Z: %.2f > %.2f ? %d \n\n", it->z, highest_points[x][y], (it->z > highest_points[x][y]));
            // try{

            //     // std::cout << "X: " << x << " Y: " << y << " point " << highest_points[x][y] << std::endl;
            //     // std::cout << "Z: " << it->z << " ?: " << (it->z > highest_points[x][y]) << std::endl;

            //     if ( !std::isnan(it->z) && it->z > highest_points[x][y])
            //     {
            //         highest_points[x][y] = it->z;
            //         // std::cout << "Z: " << it->z << std::endl;
            //         // std::cout << "New highest point: " << highest_points[x][y] << std::endl << std::endl;
            //     }
            // }catch(...){
            //     continue;
            // }
            ...
            // for(std::vector< std::vector<float> >::iterator row = highest_points.begin(); row != highest_points.end(); ++row)
            // {
