...
class AselsanRos : public rclcpp::Node
{

public:

    AselsanRos(Model model) : model(model), Node("aselsan_ros"){
        ...
        cmd_vel_sub = this->create_subscription<geometry_msgs::msg::Twist>(
            "cmd_vel", 10, std::bind(&AselsanRos::cmd_vel_callback, this, std::placeholders::_1));

        odom_sub = this->create_subscription<nav_msgs::msg::Odometry>(
            "odom/ground_truth", 10, std::bind(&AselsanRos::odom_callback, this, std::placeholders::_1));

        joint_state_sub = this->create_subscription<sensor_msgs::msg::JointState>(
            "joint_states", 10, std::bind(&AselsanRos::joint_state_callback, this, std::placeholders::_1));
        ...
        timer_ = create_wall_timer(std::chrono::milliseconds(2), std::bind(&AselsanRos::run_model, this));
...

{
    ...
    ModelParams aselsan_params;
    aselsan_params.num_observations = 48;
    aselsan_params.clip_obs         = 100.0;
    aselsan_params.clip_actions     = 100.0;                                             
    aselsan_params.damping          = 1;
    aselsan_params.stiffness        = 40;
    aselsan_params.d_gains          = torch::ones(12)*aselsan_params.damping;
    aselsan_params.p_gains          = torch::ones(12)*aselsan_params.stiffness;
    aselsan_params.action_scale     = 0.25;
    aselsan_params.num_of_dofs      = 12;
    aselsan_params.lin_vel_scale    = 2.0;
    aselsan_params.ang_vel_scale    = 0.25;
    aselsan_params.dof_pos_scale    = 1.0;
    aselsan_params.dof_vel_scale    = 0.05;
    aselsan_params.commands_scale   = torch::tensor({aselsan_params.lin_vel_scale, aselsan_params.lin_vel_scale, aselsan_params.ang_vel_scale});

                                                   //hip, thigh, calf
    aselsan_params.torque_limits    =   torch::tensor({{20.0, 55.0, 55.0,     // front left
                                                   20.0, 55.0, 55.0,     // front right
                                                   20.0, 55.0, 55.0,     // rear  left
                                                   20.0, 55.0, 55.0 }}); // rear  right

                                                 
    aselsan_params.default_dof_pos  = torch::tensor({{ 0.1000,  0.8000, -1.5000,    
    ...
    Model model(share_directory + "/models/aselsan.pt", aselsan_params);

    rclcpp::init(argc, argv);
    rclcpp::spin(std::make_shared<AselsanRos>(model));
    rclcpp::shutdown();
